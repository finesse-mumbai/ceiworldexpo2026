...
import { User, Mail, Building, Briefcase, Phone, MapPin, Globe, AlertCircle } from 'lucide-react';
...
const knowOptions = [
  ...
  { value: "Bunting/Billboard", label: "Social Media" }
];
...
export default function BuyerRegistrationForm({ defaultUtmSource, pageTitle }: BuyerRegistrationFormProps) {
  ...
  const [txt_state, setTxtState] = useState<string>(''); 
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const myUtmSource = urlParams.get('utm_source');
      if (myUtmSource) {
        setUtmSource(myUtmSource);
      }
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    params.append('utm_source', utmSource); // UTM source tracking parameter
            ...
            <div className="lg:col-span-3 bg-gradient-to-b from-[#009ad7] to-[#007ba8] backdrop-blur-xl border-b lg:border-b-0 lg:border-r border-slate-200/50 p-8 md:p-10 flex flex-col justify-start items-center text-center relative overflow-hidden min-h-[250px] lg:min-h-[600px] lg:pt-16 z-10">
              ...
              <div className="relative z-10 flex flex-col items-center">
                <h2 className="text-2xl lg:text-3xl font-black leading-tight tracking-wider uppercase font-sans">
                  <span className="text-white">Buyer</span> <br /> <span className="text-white">Registration</span>
                </h2>
              </div>
            </div>
            ...
            <div className="lg:col-span-9 p-6 md:p-10 bg-slate-50/40 z-10">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-blue-50 border-l-4 border-[#009ad7] p-4 rounded-r-lg mb-6 shadow-sm">
                  <div className="flex items-start">
                    <div className="flex-shrink-0 mt-0.5">
                      <AlertCircle className="h-5 w-5 text-[#009ad7]" />
                    </div>
                    <div className="ml-3">
                      <p className="text-sm text-black-900 font-medium">
                        <strong className="font-bold">Important Note:</strong> Entry is strictly limited to trade buyers upon presentation of a valid business card. Individuals under the age of 18 will not be permitted. Registration is free of charge. The right to grant or deny admission is reserved.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-b border-slate-100 pb-6">
                  <TitleSelector value={title} onChange={setTitle} error={errors.title} name="buyer-title" />
                </div>
                ...
                <div className="pt-6 flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="text-left text-sm text-slate-500 leading-relaxed mt-4 w-full border-t border-slate-100 pt-4">
                    <strong>Note:</strong> For buyer registrations and related inquiries, please reach out to <strong>Akash Prabhu</strong> at <strong>Mobile: <a href="https://wa.me/919137587951?text=Hi%2C%20I%20have%20a%20query%20regarding%20Buyer%20Registration%20at%20CEI%20World%20Expo%202026%3A%0A" target="_blank" rel="noopener noreferrer" className="hover:text-[#009ad7] hover:underline transition-colors" title="Chat on WhatsApp">+91-9137587951</a> </strong> or via email at <strong> <a href="mailto:akash@worldexindia.com?subject=Query%20regarding%20Buyer%20Registration%20-%20CEI%20World%20Expo%202026" className="hover:text-[#009ad7] hover:underline transition-colors" title="Send an Email">akash@worldexindia.com</a></strong>
                  </p>
                </div>
              </form>
            </div>
