...
export default function FairInfo() {
  ...
  const splitWords = (phrase: string) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      ...
      body.push(
        <span key={word + "_" + i} className={`inline-block ${isBold ? 'font-black' : 'font-medium'}`} style={{ paddingRight: '0.25em' }}>
          {letters}
        </span>
      );
    });
  ...
  return (
    <section className="pt-32 md:pt-40 pb-20 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div
          ref={container}
          className="text-xl md:text-2xl lg:text-4xl font-medium leading-[1.6] text-black text-center mx-auto w-full max-w-5xl px-2 [text-wrap:balance]"
        >
          {splitWords(phrase)}
        </div>
